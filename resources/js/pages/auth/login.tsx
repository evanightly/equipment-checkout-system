...
export default function Login({ status, canResetPassword }: LoginProps) {
    ...
    useEffect(() => {
        // Check if login field contains email format
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        setIsEmail(emailRegex.test(data.login));
    }, [data.login]);
        ...
        <>
            ...
            <div className='relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-purple-100 to-slate-100 dark:from-slate-900 dark:via-purple-900 dark:to-slate-900'>
                        ...
                        >
                            ...
                            <div className='relative rounded-[16px] p-8 backdrop-blur-sm'>
                                ...
                                <form className='space-y-6' onSubmit={submit}>
                                    ...
                                    >
                                        <Label className='mb-2 block text-slate-700 dark:text-slate-200' htmlFor='login'>
                                            {isEmail ? 'Email' : 'NIP / Email'}
                                        </Label>
                                        <div className='group relative'>
                                            <Input
                                                ...
                                                placeholder={isEmail ? 'your.email@company.com' : 'Enter 18-digit NIP or email'}
                                                type='text'
                                                value={data.login}
                                            />
                                            <div className='absolute top-1/2 left-3 -translate-y-1/2 transform text-purple-400'>
                                                {isEmail ? <Mail size={18} /> : <User size={18} />}
                                            </div>
                                        </div>
                                        <InputError className='mt-2 text-pink-400' message={errors.login} />
                                    </motion.div>
                                    ...
                                    >
                                        <div className='flex items-center space-x-2'>
                                            <Checkbox
                                                ...
                                                className='border-purple-400/50 data-[state=checked]:border-purple-600 data-[state=checked]:bg-purple-600'
                                                id='remember'
                                    ...
                                    >
                                        <div className='text-sm text-slate-600 dark:text-slate-300'>
                                            ...
                                            <TextLink className='text-purple-400 transition-colors hover:text-purple-300' href={route('register')}>
                                                Create account
                                            </TextLink>
                                        </div>
                                    </motion.div>
                ...
                {status && (
                    ...
                    >
                        <div className='rounded-lg border border-green-400/30 bg-green-500/10 px-6 py-3 text-green-300 shadow-lg backdrop-blur-sm'>
                            {status}
                        </div>
                    </motion.div>
                )}
