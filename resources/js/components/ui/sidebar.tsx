...
type SidebarContextProps = {
    ...
    collapsibleMode: 'offcanvas' | 'icon';
    toggleCollapsibleMode: () => void;
};
...
function SidebarProvider({
    ...
    // Add state for collapsible mode
    const [collapsibleMode, setCollapsibleMode] = React.useState<'offcanvas' | 'icon'>('offcanvas');
    ...
    // Toggle function for collapsible mode
    const toggleCollapsibleMode = React.useCallback(() => {
        setCollapsibleMode((prev) => (prev === 'offcanvas' ? 'icon' : 'offcanvas'));
        // When switching to icon mode, ensure sidebar is expanded
        // if (collapsibleMode === 'offcanvas') {
        //     setOpen(true);
        // }
    }, []);
    ...
    const contextValue = React.useMemo<SidebarContextProps>(
        () => ({
            ...
            toggleSidebar,
            collapsibleMode,
            toggleCollapsibleMode,
        }),
        [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, collapsibleMode, toggleCollapsibleMode],
    );
...
function Sidebar({
    ...
    // Get the collapsible mode from context, but use prop as fallback
    const { isMobile, state, openMobile, setOpenMobile, collapsibleMode: contextCollapsibleMode } = useSidebar();

    // Use the prop if provided, otherwise use the context value
    const collapsible = collapsibleProp !== 'none' ? contextCollapsibleMode : collapsibleProp;
...
const sidebarGroupLabelVariants = cva(
    'text-sidebar-foreground/70 ring-sidebar-ring flex shrink-0 items-center outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:shrink-0 group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0',
    {
        variants: {
            variant: {
                default: 'h-8 rounded-md px-2 text-xs font-medium [&>svg]:size-4',
                windui: 'text-sm p-2 my-2 text-foreground/70',
            },
        },
        defaultVariants: {
            variant: 'default',
...
function SidebarCollapsibleModeTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
    const { toggleCollapsibleMode, collapsibleMode } = useSidebar();
    ...
    return (
        <Button
            ...
            {...props}
        >
            {collapsibleMode === 'offcanvas' ? <PanelRightClose /> : <PanelLeftClose />}
            <span className='sr-only'>Toggle Sidebar Mode ({collapsibleMode === 'offcanvas' ? 'Icon Mode' : 'Normal Mode'})</span>
        </Button>
    );
}
